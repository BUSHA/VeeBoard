...
const [boardId, name, pinCode, avatarUrl = "", avatarKey = ""] = process.argv.slice(2);

if (!boardId || !name || !pinCode) {
  console.error("Usage: node cloudflare-worker/scripts/provision-user.mjs <boardId> <name> <pinCode> [avatarUrl] [avatarKey]");
  process.exit(1);
}
...
console.log(`INSERT OR REPLACE INTO board_users (board_id, name, avatar_url, avatar_key, updated_at) VALUES ('${sqlEscape(boardId)}', '${sqlEscape(name)}', '${sqlEscape(avatarUrl)}', '${sqlEscape(avatarKey)}', '${sqlEscape(now)}');`);
